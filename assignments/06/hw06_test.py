...
try:
    ...
    print(
        "tqdm was not found, using alternative progress bar. To install, use `$ pip install tqdm`\n"
    )
    import shutil
    ...
    def tqdm(iterable):
        ...
        for i, x in enumerate(iterable):
            ...
            bar = (
                "["
                + (int(available * itr / len(iterable)) - 1) * "="
                + ">" * (end == "\r")
            )
...
def timeit(func):
    def timed_func(*args, **kwargs):
        ...
        test_output.append(
            f'PASSED {func.__name__}{" "*(21-len(func.__name__))} in {(runtime):>8} ms'
        )
        return out
...
def test8_timeout_tests():
    if not TIMEOUT_TEST:
        ...
        return
    print(
        "Running Timeout Tests. These can take a long time if your code is not efficient or your chosen language is slow.",
        end=" ",
    )
    print(
        f"Set TIMEOUT_TEST=False on line 40 if you don't want to test this, or set MAX_VALUE (currently {MAX_VALUE}) on line 41 to be a smaller number."
    )
    tests = dict()
    increasing = list(map(str, range(1, MAX_VALUE + 1, 1)))
    decreasing = increasing[::-1]
    tests["timeout-test-0"] = {
        "input": f"1\n{MAX_VALUE}\n"
        + "\n".join(increasing)
        + "\n"
        + "\n".join(increasing),
    ...
    tests["timeout-test-1"] = {
        ...
        + "\n"
        + "\n".join(decreasing),
        "output": f"{int(MAX_VALUE * (MAX_VALUE - 1) / 2)}\n",
    }
...
def run_tests(tests):
    for testName, testCase in tqdm(tests.items()):
        ...
        userOutput = "\n".join(
            userOutput.split("\n")[1:]
        )  # first line is Makefile's run command
        if userOutput != testCase["output"]:
            ...
            if not all(x[0] == " " or x[-1] in whitespace for x in diff):
                print(
                    "Output difference (lines with - need to be removed, lines with + need to be added):"
                )
                sys.stdout.writelines(
                    difflib.unified_diff(
                        userOutput.splitlines(keepends=True),
...
def get_versions():
    ...
    try:
        with urlopen(
            "https://raw.githubusercontent.com/CS577-testers-SP22/hw06-tester/master/.version"
        ) as f:
            if f.status != 200:
                raise Exception
...
if __name__ == "__main__":
    ...
    if current < latest:
        print(
            f"A newer version of this tester (v{to_v_str(latest)}) is available. You are current running v{to_v_str(current)}\n"
        )
        print(
            "You can download the latest version at https://github.com/CS577-testers-SP22/hw06-tester\n"
        )
    print()
    main()
    print()
